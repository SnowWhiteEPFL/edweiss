import { EditExerciseModal } from '@/app/(app)/quiz/createQuizPage';
...
const mockRemoveExerciseFromList = jest.fn()

describe('EditExerciseModal', () => {

	it('renders scroll view modal', () => {		// Mock the handle function
		const modalRef = { current: { open: jest.fn(), close: jest.fn(), present: jest.fn(), dismiss: jest.fn(), snapToIndex: jest.fn(), snapToPosition: jest.fn(), expand: jest.fn(), collapse: jest.fn(), forceClose: jest.fn() } }; // Mock modalRef
		...
		const screen = render(<BottomSheetModalProvider>
			<EditExerciseModal index={0} secondModalRef={secondModalRef} removeExerciseFromList={mockRemoveExerciseFromList} editExercise={jest.fn()} exercise={mockTF} modalRef={modalRef} />
		</BottomSheetModalProvider>)
		expect(screen.getByTestId('scroll-view')).toBeTruthy()
	})
	it('calls remove fn ', () => {		// Mock the handle function
		const modalRef = { current: { open: jest.fn(), close: jest.fn(), present: jest.fn(), dismiss: jest.fn(), snapToIndex: jest.fn(), snapToPosition: jest.fn(), expand: jest.fn(), collapse: jest.fn(), forceClose: jest.fn() } }; // Mock modalRef
		const secondModalRef = { current: { open: jest.fn(), close: jest.fn(), present: jest.fn(), dismiss: jest.fn(), snapToIndex: jest.fn(), snapToPosition: jest.fn(), expand: jest.fn(), collapse: jest.fn(), forceClose: jest.fn() } }; // Mock modalRef
		...
		expect(mockRemoveExerciseFromList).toHaveBeenCalled()
	})
})