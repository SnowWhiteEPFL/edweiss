...
const AssignmentDisplay: ReactComponent<{ item: AssignmentWithColor, index: number, isSwipeable: boolean; }> = ({ item, index, isSwipeable }) => {
    ...
    return (
        isSwipeable ?
            <Swipeable testID={testIDs.swipeableComponent} ref={(ref) => { swipeableRefs.current[index] = ref; }} renderRightActions={renderRightActions} onSwipeableOpen={(direction) => { if (direction === 'right') { console.log(`Swipe detected on assignment: ${item.name}`); saveTodo(item.name, item.dueDate, item.type); Toast.show({ type: 'success', text1: t(`course:toast_added_to_todo_text1`), text2: t(`course:toast_added_to_todo_text2`), }); swipeableRefs.current[index]?.close(); } }}>
                {assignmentView()}
            </Swipeable>
            : assignmentView()
    );
