...
const Route: ApplicationRoute = () => {
	...
	// 				<TText mb='md' ml='md' size={'xl'}> {quizName}</TText>
...
export const AddExerciseModal: ReactComponent<{ modalRef: RefObject<BottomSheetModalMethods>, updateExerciseList: (exercise: Quizzes.Exercise) => void }> = (props) => {
	...
	return (
		...
		<ModalContainerScrollView disabledDynamicSizing snapPoints={['80%']} modalRef={props.modalRef}>

			<TView mb='lg' flexDirection='row' flexColumnGap='sm' >
				<TView flex={1}>
					<FancyButton onPress={() => setIsMCQ(true)} backgroundColor={isMCQ ? "blue" : "base"}> MCQ </FancyButton>

				</TView>
				<TView flex={1}>
					<FancyButton onPress={() => setIsMCQ(false)} backgroundColor={!isMCQ ? "blue" : "base"}> True-False </FancyButton>
				</TView>
...
export const MCQFields: ReactComponent<{ addToExerciseList?: (exercise: Quizzes.Exercise) => void, editExercise?: (exercise: Quizzes.Exercise, index: number) => void, modalRef: RefObject<BottomSheetModalMethods>, previousPropositions?: [string, boolean][], previousQuestion?: string, index?: number }> = (props) => {
	...
	return (<>
		<TView mb='sm'>
			...
			<FancyTextInput placeholder={placeHolder} onChangeText={setQuestion} label='Question' mb='sm'></FancyTextInput>
			{
				propositions.length == 0 ?
					<TText align='center' mb='xl' mt='xl'> No options added!</TText> :
					<For each={propositions}>{
						(proposition, index) => {
							return (<PropositionField proposition={proposition} changeProposition={changeProposition} removeProposition={removeProposition} index={index} previousProposition={props.previousPropositions == undefined ? undefined : props.previousPropositions[index]} />)
						}
					}

					</For>
			}
			<FancyButton onPress={() => addNewProposition()}> Add new proposition </FancyButton>
		</TView>
		<FancyButton onPress={() => addToQuiz()}> Save </FancyButton>
	</>
...
const PropositionField: ReactComponent<{ proposition: [string, boolean], changeProposition: (index: number, description: string, isCorrect: boolean) => void, index: number, removeProposition: (index: number) => void, previousProposition?: [string, boolean] | undefined }> = (props) => {
	...
	return (
		<TView mb='xs' flexDirection='row' flexColumnGap='xs'>
			<TView flex={1}>
				<FancyButton onPress={() => props.removeProposition(props.index)} icon='trash-bin-sharp' backgroundColor='red' />
			</TView>
			<TView flex={4}>
				<FancyTextInput label='Proposition' onChangeText={changeDescription} placeholder={props.previousProposition?.[0]} />
			</TView>
			<TView flex={1}>
				<FancyButton onPress={() => toggleCorrect()} icon='checkmark-circle-sharp' backgroundColor={props.proposition[1] ? 'green' : 'surface0'}></FancyButton>
			</TView>
		</TView>
	);
};
