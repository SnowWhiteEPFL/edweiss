...
@REM for /r ../edweiss-app/model/ %%f in (*) do ( icacls "%%f" /deny *S-1-1-0:(W) )
@echo Model updated in application.

rmdir /S /Q ../edweiss-web/src/model
xcopy "src\." "../edweiss-web/src/model/" /E /I
@REM for /r ../edweiss-web/src/model/ %%f in (*) do ( icacls "%%f" /deny *S-1-1-0:(W) )
...
@REM for /r ../edweiss-firebase/functions/src/model/ %%f in (*) do ( icacls "%%f" /deny *S-1-1-0:(W) )
